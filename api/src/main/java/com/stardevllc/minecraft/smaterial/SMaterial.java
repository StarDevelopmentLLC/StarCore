...
public enum SMaterial {
    ...
    private static Optional<SMaterial> matchSMaterialWithData(String name) {
        ...
        if (index != -1) {
            ...
            try {
                ...
                return data >= 0 && data < MAX_DATA_VALUE ? matchDefinedSMaterial(mat, data) : matchDefinedSMaterial(mat, UNKNOWN_DATA_VALUE);
            } catch (NumberFormatException ignored) {
                return matchDefinedSMaterial(mat, UNKNOWN_DATA_VALUE);
            }
        }
    ...
    public static Optional<SMaterial> matchSMaterial(String name) {
        if (name == null || name.isEmpty()) {
            throw new IllegalArgumentException("Cannot match a material with null or empty material name");
        }
        Optional<SMaterial> oldMatch = matchSMaterialWithData(name);
        return oldMatch.isPresent() ? oldMatch : matchDefinedSMaterial(format(name), UNKNOWN_DATA_VALUE);
    }
    
    public static SMaterial matchSMaterial(Material material) {
        Objects.requireNonNull(material, "Cannot match null material");
        return matchDefinedSMaterial(material.name(), UNKNOWN_DATA_VALUE)
                .orElseThrow(() -> new IllegalArgumentException("Unsupported material with no data value: " + material.name()));
    }
    
    public static SMaterial matchSMaterial(ItemStack item) {
        ...
        Optional<SMaterial> result = matchDefinedSMaterial(material, data);
        if (result.isPresent()) {
            return result.get();
        }
    ...
    private static Optional<SMaterial> matchDefinedSMaterial(String name, byte data) {
        ...
        SMaterial oldXMaterial = requestOldSMaterial(name, data);
